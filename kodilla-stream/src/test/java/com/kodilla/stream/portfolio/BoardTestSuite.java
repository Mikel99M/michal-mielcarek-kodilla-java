...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        long numberOfdays = project.getTaskLists().stream()
                ...
                .map(data -> data.getCreated())
                .map(data -> data.getDayOfYear() - LocalDate.now().getDayOfYear())
                .reduce(0, Integer::sum);

        long numberOfUsers = project.getTaskLists().stream()
                .flatMap(task -> task.getTasks().stream())
                .map(Task::getAssignedUser)
                .count();

        numberOfdays *= -1;
        long result = numberOfdays / numberOfUsers;

        //Then
        assertEquals(85, numberOfdays);
